...
namespace ChatSys
{
    ...
    public class MessageServer3
    {
        ...
        public static async void SendMessageAsync(string pipeName3, string message3, string LOCAL_SERVER, int int_Server_Send_Sleep_Time, int int_Server_DEFAULT_TIME_OUT)

        {
            try
            {
                ...
                using (var pipe =
                    new NamedPipeClientStream(LOCAL_SERVER, pipeName3, PipeDirection.InOut, PipeOptions.Asynchronous))
                using (var stream = new StreamWriter(pipe))
                {
                    ...
                    // write the message to the pipe stream 
                     await stream.WriteAsync(message3);
                }
            }
            catch (Exception exception)
            {
                OnSendMessageException(pipeName3, new MessengerExceptionEventArgs(exception));
            }
        }
