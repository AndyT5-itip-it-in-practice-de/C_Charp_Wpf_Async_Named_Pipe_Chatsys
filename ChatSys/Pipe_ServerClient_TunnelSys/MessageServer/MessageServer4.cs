...
namespace ChatSys
{
    ...
    public class MessageServer4
    {
        ...
        public static async void SendMessageAsync(string pipeName4, string message4, string LOCAL_SERVER, int int_Server_Send_Sleep_Time, int int_Server_DEFAULT_TIME_OUT)

        {
            try
            {
               ...
               //string LOCAL_SERVER = ((MainWindow)Application.Current.MainWindow).txtbx_Server_LOCAL_SERVER.Text;
                ...
                using (var pipe =
                    new NamedPipeClientStream(LOCAL_SERVER, pipeName4, PipeDirection.InOut, PipeOptions.Asynchronous))
                using (var stream = new StreamWriter(pipe))
                {
                    ...
                    await stream.WriteAsync(message4);
                }
            }
            catch (Exception exception)
            {
                OnSendMessageException(pipeName4, new MessengerExceptionEventArgs(exception));
            }
        }
