...
namespace ChatSys
{
    ...
    public class MessageServer7
    {
        ...
        public static async void SendMessageAsync(string pipeName7, string message7, string LOCAL_SERVER, int int_Server_Send_Sleep_Time, int int_Server_DEFAULT_TIME_OUT)

        {
            try
            {
                ...
                using (var pipe =
                    new NamedPipeClientStream(LOCAL_SERVER, pipeName7, PipeDirection.InOut, PipeOptions.Asynchronous))
                using (var stream = new StreamWriter(pipe))
                {
                    ...
                    await stream.WriteAsync(message7);
                }
            }
            catch (Exception exception)
            {
                OnSendMessageException(pipeName7, new MessengerExceptionEventArgs(exception));
            }
        }
