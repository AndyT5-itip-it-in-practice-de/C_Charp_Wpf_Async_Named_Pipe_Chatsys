...
namespace ChatSys
{
    ...
    public class MessageServer5
    {
        ...
        public static async void SendMessageAsync(string pipeName5, string message5, string LOCAL_SERVER, int int_Server_Send_Sleep_Time, int int_Server_DEFAULT_TIME_OUT)

        {
            try
            {
                ...
                using (var pipe =
                    new NamedPipeClientStream(LOCAL_SERVER, pipeName5, PipeDirection.InOut, PipeOptions.Asynchronous))
                using (var stream = new StreamWriter(pipe))
                {
                    ...
                    await stream.WriteAsync(message5);
                }
            }
            catch (Exception exception)
            {
                OnSendMessageException(pipeName5, new MessengerExceptionEventArgs(exception));
            }
        }
