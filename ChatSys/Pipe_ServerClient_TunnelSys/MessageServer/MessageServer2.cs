...
namespace ChatSys
{
    ...
    public class MessageServer2
    {
        ...
        public static async void SendMessageAsync(string pipeName2, string message2, string LOCAL_SERVER, int int_Server_Send_Sleep_Time, int int_Server_DEFAULT_TIME_OUT)
        {
            try
            {
                //----------------------------------------------------------------------------------------------------
                ...
                using (var pipe =
                    new NamedPipeClientStream(LOCAL_SERVER, pipeName2, PipeDirection.InOut, PipeOptions.Asynchronous))
                using (var stream = new StreamWriter(pipe))
                {
                   ...
                   //int int_Server_Send_Sleep_Time = Convert.ToInt32(Server_Send_Sleep_Time);
                    ...
                    await stream.WriteAsync(message2);
                }
            }
            catch (Exception exception)
            {
                OnSendMessageException(pipeName2, new MessengerExceptionEventArgs(exception));
            }
        }
