...
namespace ChatSys
{
    ...
    public class MessageServer6
    {
        ...
        public static async void SendMessageAsync(string pipeName6, string message6, string LOCAL_SERVER, int int_Server_Send_Sleep_Time, int int_Server_DEFAULT_TIME_OUT)

        {
            try
            {
                ...
                using (var pipe =
                    new NamedPipeClientStream(LOCAL_SERVER, pipeName6, PipeDirection.InOut, PipeOptions.Asynchronous))
                using (var stream = new StreamWriter(pipe))
                {
                    ...
                    await stream.WriteAsync(message6);
                }
            }
            catch (Exception exception)
            {
                OnSendMessageException(pipeName6, new MessengerExceptionEventArgs(exception));
            }
        }
